...
void SQLiteDB::GetTableInfo(const string &table_name, ColumnList &columns,
    ...
    auto primary_key_index = idx_t(-1);
...
bool SQLiteDB::ColumnExists(const string &table_name, const string &column_name) {
    ...
    stmt = Prepare(
            StringUtil::Format("PRAGMA table_info(\"%s\")", SQLiteUtils::SanitizeIdentifier(table_name)));
    while (stmt.Step()) {
        auto sqlite_colname = stmt.GetValue<string>(1);
...
bool SQLiteDB::GetMaxRowId(const string &table_name, idx_t &max_row_id) {
    SQLiteStatement stmt;
    if (!TryPrepare(
            StringUtil::Format("SELECT MAX(ROWID) FROM \"%s\"", SQLiteUtils::SanitizeIdentifier(table_name)),
            stmt)) {
        return false;
    }
    if (!stmt.Step()) {
...
vector <IndexInfo> SQLiteDB::GetIndexInfo(const string &table_name) {
    ...
    for (auto &index_name: unique_indexes) {
        stmt = Prepare(
                StringUtil::Format("SELECT cid FROM pragma_index_info('%s')",
                                   SQLiteUtils::SanitizeString(index_name)));
